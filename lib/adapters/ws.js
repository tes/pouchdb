...
var RPC = require('../deps/ws-rpc');
var log = require('debug')('pouchdb:ws:log');
...
// Implements the PouchDB API for dealing with CouchDB instances over WebSockets
function WSPouch(opts, callback) {
  // The functions that will be publicly available for WSPouch
  var api = this;
  ...
  var rpcOpts = opts.rpc || {};
  opts = utils.clone(opts);
  function rpc(options, callback) {
    var reqOpts = utils.extend({}, rpcOpts, options);
    return RPC(reqOpts, callback);
  }

  // Create a new CouchDB database based on the given opts
  var createDB = function () {
    rpc({headers: host.headers, method: 'PUT', url: dbUrl},
         function (err, ret) {
      // If we get an "Unauthorized" error
      if (err && err.status === 401) {
        // Test if the database already exists
        rpc({headers: host.headers, method: 'HEAD', url: dbUrl},
             function (err, ret) {
  ...
  if (!opts.skipSetup) {
    rpc({headers: host.headers, method: 'GET', url: dbUrl},
         function (err, ret) {
  ...
  api.get = utils.adapterFun('get', function (id, opts, callback) {
    ...
    // Set the options for the rpc call
    var options = {
      headers: host.headers,
      method: 'GET',
    ...
    // Get the document
    rpc(options, function (err, doc, xhr) {
  ...
  api._changes = function (opts) {
    ...
    var fetched = function (err, res) {
      ...
      // If the result of the rpc call (res) contains changes (res.results)
      if (res && res.results) {
...
// Delete the WSPouch specified by the given name.
WSPouch.destroy = utils.toPromise(function (name, opts, callback) {
  ...
  var rpcOpts = opts.rpc || {};
  opts = utils.extend({}, opts, rpcOpts);
  utils.rpc(opts, callback);
});

// WSPouch is a valid adapter.
WSPouch.valid = function () {
  return true;
};

module.exports = WSPouch;
